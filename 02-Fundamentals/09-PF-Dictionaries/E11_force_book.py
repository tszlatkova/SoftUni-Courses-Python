...
def check_for_user(dict_users: dict, user: str):
    """
    Check if a given user is in some of the forces.
    """
    for key in dict_users.keys():
        for name in dict_users[key]:
            if name == user:
                return True
...
def remove_user(dict_users: dict, user_to_remove: str):
    """
    Remove a user from his current force.
    """
    for key in dict_users.keys():
        for name in dict_users[key]:
            if name == user_to_remove:
                dict_users[key].remove(user_to_remove)
...
while input_line != 'Lumpawaroo':
    if '|' in input_line:
        information = input_line.split(' | ')
        force_side = information[0]
        force_user = information[1]

        if not check_for_user(forces, force_user):
        ...
        information = input_line.split(' -> ')
        force_side = information[1]
        force_user = information[0]

        if force_side not in forces.keys():
            forces[force_side] = []

        if check_for_user(forces, force_user):
...
for force in forces.keys():
    if len(forces[force]) > 0:
        print(f'Side: {force}, Members: {len(forces[force])}')
        for user in forces[force]:
            print(f'! {user}')
