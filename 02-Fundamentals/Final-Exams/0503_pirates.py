...
def plundering(cities: dict, town_name, town_population, stolen_gold):
    cities[town_name]['population'] -= town_population
    cities[town_name]['gold'] -= stolen_gold
...
def prosper(cities: dict, town_name, gold_added):
    cities[town_name]['gold'] += gold_added
...
while True:
    ...
    if city not in city_data.keys():
        city_data[city] = {'population': int(population), 'gold': int(gold)}
    else:
        city_data[city]['population'] += int(population)
...
while True:
    ...
    if action == 'Plunder':
        ...
        plundering(city_data, town, people, gold)
        ...
        if gold < 0:
            ...
            prosper(city_data, town, gold)
            total_town_gold = city_data[town]['gold']
            print(f'{gold} gold added to the city treasury. {town} now has {total_town_gold} gold.')

if len(city_data) == 0:
    print(f'Ahoy, Captain! All targets have been plundered and destroyed!')
else:
    print(f'Ahoy, Captain! There are {len(city_data)} wealthy settlements to go to:')

    for town in city_data:
        gold = city_data[town]['gold']
        people = city_data[town]['population']
