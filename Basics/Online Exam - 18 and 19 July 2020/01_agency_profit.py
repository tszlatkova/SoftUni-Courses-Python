...
# броя продадени детски билети. Нетната цена на билета за възрастен се определя от авиокомпанията, а детският билет
# е със 70% по-евтин. Агенцията добавя към нетната цена на всеки билет такса обслужване.  Крайната печалба на Агенцията
# е 20% от общата цена на всички билети.
# Вход:
...
name = input()
elderly_number = int(input())
kids_number = int(input())
net_price_elderly = float(input())
service_price = float(input())

total_price = ( elderly_number + kids_number  *  0.30 ) * net_price_elderly \
              + (elderly_number + kids_number) * service_price

print(f'The profit of your agency from {name} tickets is {0.20 * total_price:.2f} lv.')