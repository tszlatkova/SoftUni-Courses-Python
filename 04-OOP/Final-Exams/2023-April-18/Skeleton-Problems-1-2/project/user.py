class User:
    ...
    RATING_INCREASE_BY = 0.5
    RATING_MAX_VALUE = 10
    RATING_DECREASE_BY = 2.0
    ...
    def increase_rating(self):
        self.rating += self.RATING_INCREASE_BY
        if self.rating > self.RATING_MAX_VALUE:
            self.rating = self.RATING_MAX_VALUE

    def decrease_rating(self):
        expected_rating = self.rating - self.RATING_DECREASE_BY

        if expected_rating < 0:
            self.rating = 0
            self.is_blocked = True
        else:
            self.rating -= self.RATING_DECREASE_BY
