...
class Pizza:
    ...
    def max_number_of_toppings(self, value):
        if value <= 0:
            raise ValueError("The maximum number of toppings cannot be less or equal to "
                             "zero")
    ...
    def calculate_total_weight(self):
        weight_toppings = [self.toppings[t] for t in self.toppings.keys()]
        total_weight = sum(weight_toppings) + self.dough.weight
        return total_weight
