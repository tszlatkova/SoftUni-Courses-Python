...
# •	is_name_valid(name) - returns whether the name is greater than or equal to the
# min_length (True/False)

# •	is_mail_valid(mail) - returns whether the mail is in the possible mails list
# (True/False)

# •	is_domain_valid(domain) - returns whether the domain is in the possible domains list
# (True/False)

# Create one public method:
# •	validate(email) - using the three methods returns whether the email is valid
# (True/False)
...
class EmailValidator:
    ...
    def validate(self, email) -> bool:
        username, not_username = email.split('@')
        mail, domain = not_username.split('.')
